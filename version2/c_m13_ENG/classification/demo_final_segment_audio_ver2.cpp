 ...
 */
 /*
 2024/8/21 - newest version, change the cutting point.
 */
 /*
 ver2:
 Don't crop the margin because the input image doesn't exist margin.
 And, the input image's size is (216, 128)
 */
	...
	vector<Mat> inputs;
 	...
 	double fake_value = 0, real_value = 0;
  	for (long unsigned int i = 0, j = -1; i < imgs.size(); i++) {
		...
		cv::Mat input_image = imgs[i];
		
		//cv::imwrite("cropped.jpg", input_image);
		// (2) Resize it if its size is not match.
		cv::Mat image; // accept the image after resized
		
		input_cols.push_back(input_image.cols);
		input_rows.push_back(input_image.rows);
		cout << "Before resize " << input_image.size() << endl;
		if (size != input_image.size()) {
			//input_image.convertTo(input_image, CV_32F, 1.0 / 255.0);
		  	cv::resize(input_image, image, size, 0, 0, cv::INTER_LINEAR); // void resize(InputArray src, OutputArray dst, Size dsize, double fx=0, double fy=0, int interpolation=INTER_LINEAR );
