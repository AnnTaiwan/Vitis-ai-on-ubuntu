...
void plot_mel_spectrogram(const std::vector<std::vector<float>>& mel_spec, const std::string& output_file) {
    ...
    int desired_width = 496;  // Desired width of the output image
    int desired_height = 371; // Desired height of the output image

    // Create a surface with the desired size
    cairo_surface_t *surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, desired_width, desired_height);
    ...
    float x_scale = static_cast<float>(desired_width) / num_frames;
    float y_scale = static_cast<float>(desired_height) / num_mels;
    ...
    for (int y = 0; y < num_mels; ++y) {
        for (int x = 0; x < num_frames; ++x) {
            ...
            cairo_rectangle(cr, x * x_scale, desired_height - y * y_scale, x_scale, y_scale);
            cairo_fill(cr);
        }
    }
...
int main(int argc, char *argv[]) {
    if(argc < 3) {
        std::cout << "Error: Please add the txt files after the execution command, like ./plot_mel_spec_from_txt_scale <files.txt> <image.png>" << std::endl;
        return 0;
    }
    std::string file_path = argv[1];
    int num_mels = 128;  // Number of Mel bands
    int num_frames = 216;  // Number of frames
