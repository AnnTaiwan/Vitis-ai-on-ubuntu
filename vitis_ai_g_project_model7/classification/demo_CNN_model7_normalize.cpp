  	...
  	for (int i = 0; i < count; i++) 
  	{
        ...
        if (x >= 0 && x < image.cols && y >= 0 && y < image.rows) {
            if (image.channels() == 1) {
                ...
                // Color image (BGR)
                cv::Vec3f pixel_value = image.at<cv::Vec3b>(y, x);
                float blue = pixel_value[0];
                float green = pixel_value[1];
                float red = pixel_value[2];
                std::cout << "0 Pixel value at (" << x << ", " << y << "): "
                          << "Blue = " << blue << ", "
                          << "Green = " << green << ", "
                          << "Red = " << red << std::endl;
            }
 	...
 	vector<int> input_cols, input_rows;
  	for (long unsigned int i = 0, j = -1; i < imgs.size(); i++) {
        ...
        if (x >= 0 && x < cropped_image.cols && y >= 0 && y < cropped_image.rows) {
            if (cropped_image.channels() == 1) {
                // Grayscale image
                float pixel_value = cropped_image.at<uchar>(y, x);
                std::cout << "#####Pixel value at (" << x << ", " << y << "): " << pixel_value << std::endl;
            } else if (cropped_image.channels() == 3) {
                // Color image (BGR)
                cv::Vec3f pixel_value = cropped_image.at<cv::Vec3b>(y, x);
                float blue = pixel_value[0];
                float green = pixel_value[1];
                float red = pixel_value[2];
                std::cout << "1 Pixel value at (" << x << ", " << y << "): "
                          << "Blue = " << blue << ", "
                          << "Green = " << green << ", "
                          << "Red = " << red << std::endl;
            }
		...
		*/
		...
		///////////////////////////
		// Specify the pixel coordinates again
        x = 0; // Column index
        y = 0; // Row index
        ...
        if (x >= 0 && x < image.cols && y >= 0 && y < image.rows) {
            if (image.channels() == 1) {
                ...
                // Color image (BGR)
                cv::Vec3f pixel_value = image.at<cv::Vec3f>(y, x);
                float blue = pixel_value[0];
                float green = pixel_value[1];
                float red = pixel_value[2];
                std::cout << "3 Pixel value at (" << x << ", " << y << "): "
                          << "Blue = " << blue << ", "
                          << "Green = " << green << ", "
                          << "Red = " << red << std::endl;
            }
        ...
        if (x >= 0 && x < image.cols && y >= 0 && y < image.rows) {
            if (image.channels() == 1) {
                ...
                float green = pixel_value[1];
                float red = pixel_value[2];
                std::cout << "############## 44 Pixel value at (" << x << ", " << y << "): "
                          << "Blue = " << blue << ", "
                          << "Green = " << green << ", "
                          << "Red = " << red << std::endl;
            }
		...
		// Retrieve the first output tensor (e.g., class probabilities)
		/*output_tensors = task->getOutputTensor(1u); // 'u' is unsigned
		cout << i << "th: Finish image=> " << imgs_names[i] << endl;
		cout << "Output tensor's size: " << output_tensors.size() << endl; // only 1 [batch_size, 
		for(int x = 0; x < output_tensors.size(); x++){
			cout << output_tensors[x] << " ";
			...
			// output [ CNN_model6__CNN_model6_Sequential_class_layers__ModuleList_0__Linear_1__248_fix ] 
			// {, size=2, h=1, w=1, c=2, fixpos=4, virt= (0xffff9b55b000 )}
			cout << "tensor_scale" << tensor_scale(output_tensors[x]) << endl;
		}
		cout << endl;
		*/
		////////////////////////
